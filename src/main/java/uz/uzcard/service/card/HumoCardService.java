...
public class HumoCardService {
    ...
    public void plusCommission(Long commission) {
        cardRepository.plusCommission(humoCardPan, commission);
    }

    public void rollbackCommission(String pan, Long amount, Long commission) {
        cardRepository.rollbackCommission(pan, amount, humoCardPan, commission);
    }

    public CardEntity getByPanOrThrow(String pan) {
        if (!pan.startsWith(issuerIdNumber)) {
            log.warn("Not Humo Card! {}", pan);
            throw new AppBadRequestException("Not Humo Card!");
        }
